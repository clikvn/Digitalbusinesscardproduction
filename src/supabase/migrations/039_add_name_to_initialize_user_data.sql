-- ============================================
-- FIX: Add name from signup to business card initialization
-- ============================================
-- Issue: When users sign up, the name field is stored in auth.users.raw_user_meta_data
-- but initialize_user_data sets the business card name to empty string
-- Solution: Fetch name from auth.users.raw_user_meta_data and use it when creating business card
...
BEGIN
  ...
  -- Name is stored in raw_user_meta_data->>'name' or raw_user_meta_data->>'full_name'
  -- Email is in the email column
  SELECT 
    COALESCE(
  ...
  -- 2. Create user_plan (default to 'free') - FIXED: use plan_name not plan_type
  ...
  -- 5. Create share settings with share_code
...
DO $$
BEGIN
  RAISE NOTICE '✅ Updated initialize_user_data to fetch name from auth.users.raw_user_meta_data';
  RAISE NOTICE '✅ Name will now be populated in business_cards.name column during signup';
  RAISE NOTICE '✅ Email will also be populated in business_cards.email column';
END $$;
